...
async def init_db():
    ...
    if db_manager.db is not None:
        await db_manager.db["logs"].create_index("tenant_id")
        await db_manager.db["security_alerts"].create_index("tenant_id")
        await db_manager.db["analysis_results"].create_index("tenant_id")
        await db_manager.db["firewall_rules"].create_index([("tenant_id", 1), ("ip", 1)])
        await db_manager.db["users"].create_index("tenant_id", unique=True)
