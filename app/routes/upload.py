...
async def analyze_log_file(request: Request, file: UploadFile = File(...), db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        print(f"\n📤 UPLOAD START")
        print(f"   Raw tenant_id: '{raw_tenant_id}'")
        print(f"   Sanitized tenant_id: '{secure_tenant_id}'")
        ...
        try:
            ...
            # Multi-mode CSV Parsing (Try comma first, fallback to semi-colon)
            csv_reader = csv.DictReader(io.StringIO(clean_content))
            if not csv_reader.fieldnames or len(csv_reader.fieldnames) < 2:
                csv_reader = csv.DictReader(io.StringIO(clean_content), delimiter=';')

            if not csv_reader.fieldnames:
                raise HTTPException(status_code=400, detail="CSV header is missing.")

            # ✅ CTO FIX: Strip whitespace from every header field to prevent lookup failures
            csv_reader.fieldnames = [f.strip() for f in csv_reader.fieldnames if f]

            resolved = resolve_columns(csv_reader.fieldnames)
            
            # --- START SMART DETECTION HOOK ---
            all_rows = list(csv_reader)
            resolved = guess_columns_by_content(all_rows, csv_reader.fieldnames, resolved)
            ...
            field_lookup = {c.lower(): c for c in (csv_reader.fieldnames or [])}

            for row in all_rows:
                # 🔄 CTO FIX: Normalize row keys by stripping whitespace
                ...
                if len(logs_batch) >= batch_size:
                    ...
                    print(f"✅ Batch inserted {len(logs_batch)} rows. IDs: {result.inserted_ids[:3]}...")
                    logs_batch.clear()

            # Insert any remaining logs
            if logs_batch:
                result = await db["csv_uploads"].insert_many(logs_batch)
                print(f"✅ Final batch inserted {len(logs_batch)} rows. IDs: {result.inserted_ids[:3]}...")

            print(f"✅ CSV Parse Complete: {parsed_rows} total events, tenant_id={secure_tenant_id}, analysis_tag={analysis_tag}")
...
async def get_analysis_result(analysis_id: str, db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if not result:
        ...
        if any_result:
            print(f"   ❌ Analysis record was filtered from response unexpectedly!")
        else:
            print(f"   ❌ Analysis record not found for requesting tenant!")
        raise HTTPException(status_code=404, detail="Report not found")

    # Get analysis metadata
    tag = result.get("analysis_tag")
    total_events = result.get("total_events", 0)
    print(f"   ✅ Found analysis: tag={tag}, total_events={total_events}")
    ...
    if tag:
        ...
        print(f"   📊 Query csv_uploads: {query}")
