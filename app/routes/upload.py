...
async def analyze_log_file(file: UploadFile = File(...), db=Depends(get_db), current_user=Depends(get_current_user)):
    try:
        ...
        # ✅ CTO FIX 1: Stream file to disk in chunks to prevent RAM OOM crash
        ...
        try:
            # ✅ CTO FIX 2: Stream file from disk line-by-line
            with open(file_path, "r", encoding="utf-8", errors="replace") as f:
                # Discard NUL bytes on the fly
                def clean_lines(file_obj):
                    for line in file_obj:
                        yield line.replace('\x00', '')

                csv_reader = csv.DictReader(clean_lines(f))
