...
async def setup_environment(db, redis):
    ...
    # 1. Inject Tenants into Redis Plan Cache
    await redis.set(f"warsoc:tenant_plan:{TENANT_BASIC}", "basic")
    await redis.set(f"warsoc:tenant_plan:{TENANT_PRO}", "Professional")
...
async def main():
    ...
    try:
        ...
        await redis.delete(f"warsoc:tenant_plan:{TENANT_BASIC}")
        await redis.delete(f"warsoc:tenant_plan:{TENANT_PRO}")
        client.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "="*40)
        print(" FINAL BATTLE DRILL RESULTS")
        print("="*40)
        print(f"FBR COMPLIANCE: {r1}")
        print(f"SIEM ISOLATION: {r2}")
        print(f"AUDIT TRAIL:    {r3}")
        print("="*40)
        
    finally:
        # Cleanup mock tenants
        await redis.delete(f"warsoc:tenant_plan:{TENANT_BASIC}")
        await redis.delete(f"warsoc:tenant_plan:{TENANT_PRO}")
        client.close()
