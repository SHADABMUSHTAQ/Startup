...
**Launch status:** Controlled Windows SMB pilot only, with a maximum of 50 active agents per tenant and 50 aggregate active agents on the current shared host. Native Windows detection, endpoint signing, immutable archival, and hot-data exports are proven. Azure compute migration, physical retention segmentation, asynchronous archive retrieval rollout, and production code signing remain separate gates.
...
flowchart LR
    ...
    API -.->|"Archive availability metadata only"| AzureEvidence
...
2. `GET /api/v1/agent/download` returns a redirect to the exact versioned Azure artifact configured in `AGENT_CDN_URL`; the URL and manifest hash must match the approved release record.
...
5. Dashboard search queries tenant-scoped operational SIEM/alert data for a strict one-to-seven-day hot window using indexed exact fields. Historical Azure evidence requires an asynchronous retrieval request.
...
- Maximum 50 active agents per tenant and 50 aggregate active agents on the current shared host. Redis restarts cannot reset the Mongo-backed floors.
