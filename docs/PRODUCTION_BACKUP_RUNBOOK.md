...
SHA-256, and restores into a temporary MongoDB 7 container with `--network none`.
The restore data uses a temporary disk-backed Docker volume rather than host RAM.
The script records document/index counts, then removes the container and volume.
The encrypted archive is deleted by default; the non-sensitive JSON proof
remains under `tmp/backup-restore-drill`.
