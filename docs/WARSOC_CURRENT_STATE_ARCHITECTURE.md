...
| Hot/cold storage | Seven-day Mongo hot tier and immutable Azure archive | Historical retrieval is active only through the role-gated asynchronous request workflow; routine dashboard, CSV and PDF reads remain hot-tier bounded. |
...
`/api/v1/archive-retrievals` contract. Frontend commit `7816739` is live for roles
with `archive.retrieve`. It mirrors backend source/date/reason limits, renders all
worker lifecycle states, prevents same-tick duplicate requests, exposes download
actions only for `READY`, validates direct HTTPS URLs, and displays the response-
level expiry returned by the API. Automated DOM coverage proves those boundaries;
final designer polish and an authenticated role-by-role production browser
walkthrough remain separate frontend acceptance. Backend execution is active.
...
- At the August 14 checkpoint, backend `d92fb65` and frontend `6ffc9e0` formed the tested repository pair. That dated 208-test result is retained as historical evidence only and does not describe the current release or replace post-deployment identity, authenticated route, browser and resource checks.
...
- Frontend lint, the production Vite build and `npm audit` pass at commit `7816739`; the lockfile resolves patched `fflate` releases `0.6.11` and `0.8.3`. Vercel declares HSTS, CSP, clickjacking, MIME-sniffing, referrer, and browser-permission headers. The frontend uses `/incidents`, `/incidents/summary`, `/logs/live?source=siem&aggregate=true`, `/data/status`, the custom-contract quote payload, the one-time invitation-link response and the role-gated archive retrieval routes. Its Endpoint Fleet was exercised against an isolated authenticated required-signing tenant. The current production build keeps the dashboard as a separate approximately 1.44 MB minified / 445 kB gzip chunk, which remains a performance target.
...
| Archive retrieval rollout | Backend canary and the live functional frontend are accepted. Normal API hot/cold merging remains removed to protect API memory. | Complete the authenticated admin/manager/auditor/analyst production browser walkthrough, retain the canary evidence, and rehydrate only if a selected legacy object is actually Archive-tier. |
...
| Archive retrieval | Asynchronous server-side copy and direct Azure download | BACKEND PRODUCTION-PROVEN / FUNCTIONAL FRONTEND LIVE / DESIGN ACCEPTANCE OPEN | OCI release `7e9f00d` enables the isolated worker. Canary `ARCHIVE-RETRIEVAL-CANARY-20260910T133007Z-413b5717` proved allowance reservation, server-side copy, streamed SHA verification, `READY`, one direct read-only SAS download, and cleanup with the WORM source untouched. Frontend `7816739` adds exact request validation, explicit lifecycle labels, duplicate-submit protection, safe direct downloads and maintained DOM tests; its production bundle was observed live. Current SAS mode is the documented temporary `service_sas` fallback; user delegation, designer polish, and authenticated role-by-role production browser acceptance remain open. |
