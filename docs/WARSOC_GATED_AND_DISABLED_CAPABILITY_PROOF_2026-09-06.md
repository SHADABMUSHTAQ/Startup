...
| Approved 90-day Azure retention split | `PROVEN / ACTIVE` | OCI release `5c8e2dc` routes new 90-day evidence to private `warsoc-siem-90` and `warsoc-general-90` Cold containers with version-level locked WORM. Production canary `20260907T091930Z-d1187dcd` proved SIEM/general route isolation, version and streamed SHA readback, ledger-before-delete, missing-route fail-closed preservation, and Legal Hold protect/release/retry. Existing fallback blobs remain untouched. | No current activation gate. Continue daily monitoring; create 180/270/360 routes only if those durations are later approved and sold. |
