...
@pytest.mark.asyncio
async def test_runbook_rate_limiter(async_client: AsyncClient, db, redis_client, auth_headers):
    ...
    reg_resp = await async_client.post("/api/v1/agent/register", json={"activation_code": act_code, "public_key": "NXLOG"})
...
@pytest.mark.asyncio
async def test_runbook_heartbeat_sentinel(async_client: AsyncClient, db, redis_client, auth_headers):
    act_resp = await async_client.post("/api/v1/agent/generate-activation", headers=auth_headers)
    act_code = act_resp.json()["activation_code"]
    reg_resp = await async_client.post("/api/v1/agent/register", json={"activation_code": act_code, "public_key": "NXLOG"})
...
@pytest.mark.asyncio
async def test_runbook_siem_event_coverage(async_client: AsyncClient, db, redis_client, auth_headers):
