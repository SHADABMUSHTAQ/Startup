...
from app.workers.fbr_worker import _build_watch_ids as build_fbr_watch_ids


SIEM_RULE_SAMPLES = {
    "SQL_INJECTION": ("http_request", "GET /login?id=1' union select password from users --"),
    "SIEM-FW-001": ("Windows-Sec", "Windows firewall connection blocked 5156 inbound traffic"),
    ...
    "RECON_COMMANDS": ("command_line", "whoami"),
    ...
    "WEB_SHELL_ACTIVITY": ("http_request", "cmd.exe /c whoami HTTP/1.1"),
}
...
def test_peca_and_fbr_catalogs_are_complete_and_disjoint():
    ...
    assert len(peca_ids) == 9
    assert len(fbr_ids) == 7
