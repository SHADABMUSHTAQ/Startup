...
function Dashboard() {
  ...
  const { lastJsonMessage } = useWebSocket(wsUrl, {
    ...
    onError: () =>
      toast.warn("Live alerts connection lost. Reconnecting...", {
        toastId: "ws-error",
        autoClose: 3000,
      }),
  });
  ...
  useEffect(() => {
    if (lastJsonMessage && isLiveModeRef.current) {
      ...
      if (
        ...
        if (String(lastJsonMessage.severity || "").toUpperCase() === "CRITICAL") {
          toast.error(
            `🚨 Critical Threat Detected: ${lastJsonMessage.title || "Web Attack"}`,
          );
        }
      }
    }
  ...
  const {
    ...
    logs.forEach((log) => {
      ...
      if (log.time) {
        ...
        if (!Number.isNaN(date.getTime()) && date.getTime() >= twentyFourHoursAgo) {
          const bucket = Math.floor(date.getTime() / 600000) * 600000;
    ...
    let volumeArray = Object.entries(timeMap)
      .map(([bucket, value]) => ({
        ...
        name: new Date(Number(bucket)).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
        value,
    ...
    if (volumeArray.length === 0) volumeArray = [{ name: "00:00", value: 0 }];
      ...
      let prevTime = prevDate.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
          ...
          <div className="search-wrapper">
            <div className="search-field">
              ...
              <input
                ...
                placeholder="Search IP, endpoint, or threat description..."
                value={globalQuery}
            ...
            {activeTab === "dashboard" && (
              <>
                ...
                <div className="soc-analytics-grid">
                  ...
                  <div className="chart-box origin-map-card">
                    <div className="chart-title-row">
                      <h4>Attacker Origin</h4>
                      <span>{originData.reduce((sum, item) => sum + item.value, 0)} signals</span>
                    </div>
                    <div className="origin-map">
                      <div className="map-grid"></div>
                      {originData.slice(0, 5).map((origin, index) => (
                        <span
                          key={origin.name}
                          className={`map-pin pin-${index + 1}`}
                          title={`${origin.name}: ${origin.value}`}
                        >
                          <MapPin size={14} />
                        </span>
                      ))}
                    ...
                    <div className="origin-list">
                      {originData.map((origin) => (
                        <div className="origin-row" key={origin.name}>
                  ...
                  <div className="chart-box chart-box-compact operations-card">
                    <div className="chart-title-row">
              ...
              {incidentDetailLoading ? (
                ...
                <>
                  ...
                  <section className="incident-detail-section">
                    ...
                    <div className="incident-detail-grid">
                      <div><span>Rule</span><strong>{selectedIncident.incident?.rule_id || selectedIncident.incident?.ruleId || "Not recorded"}</strong></div>
                  ...
                  <section className="incident-detail-section">
                    ...
                    {selectedIncident.evidence_coverage && (
                      <div className="incident-coverage">
                        ...
                        <span>{selectedIncident.evidence_coverage.cold_archive} Azure</span>
                      ...
                      <div className="incident-evidence-list">
                        {selectedIncident.evidence.slice(0, 20).map((evidence, index) => (
                          <div className="incident-evidence-row" key={evidence.alert_uid || evidence.event_uid || evidence._id || index}>
                            ...
                            <em>{evidence.storage_tier === "cold_archive" ? "Azure archive" : "Hot storage"}</em>
                          </div>
                        ))}
                      </div>
                    )}
