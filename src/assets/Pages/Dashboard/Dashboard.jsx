...
function Dashboard() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await apiClient.get('/logs?source=uploads');
      const data = res.data;
  ...
  useEffect(() => {
    if (lastJsonMessage && isLiveModeRef.current) {
      ...
      if (
        ...
        const rawMessage =
          lastJsonMessage.title || lastJsonMessage.message || "Unknown Event";
        const msgLower = rawMessage.toLowerCase();
        const eventId =
          lastJsonMessage.event_id ||
          (lastJsonMessage.raw_data && lastJsonMessage.raw_data.event_id) ||
          0;
        ...
        setLogs((prev) => {
          const existingIndex = prev.findIndex(
            (l) => l.ip === ip && l.message === rawMessage,
          );
          if (existingIndex !== -1) {
  ...
  const handleResolveSubmit = async () => {
    if (!resolutionNotes.trim()) {
      toast.error(
        "Action Blocked: Resolution notes are strictly required by the backend.",
      );
      return;
    }
  ...
  const { volumeData, severityData } = useMemo(() => {
    ...
    volumeArray.sort(
      (a, b) =>
        new Date("1970/01/01 " + a.name) - new Date("1970/01/01 " + b.name),
    );
                ...
                <div className="bottom-split-grid">
                  <div className="logs-table-card">
                    ...
                    <div className="table-body">
                      {/* 🚀 THE UPDATED ALERT TABLE ROW WITH ACTIONS */}
                      {logs
                        .filter((l) =>
                          isLiveMode
          ...
          <div className="modal-card resolution-box">
            ...
            <div className="modal-body-content">
              <p className="incident-context">
                You are about to close the following alert. Mandatory resolution notes are required for compliance audit logs:
              </p>
              <div className="incident-target">
                <strong>{resolvingAlert.message}</strong>
                <span className="target-ip">Target IP: {resolvingAlert.ip}</span>
              </div>
