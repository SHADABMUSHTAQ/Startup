...
export default function TeamManagement() {
  ...
  const { user, role: rootRole } = useAuthStore();
  const currentUserRole = String(rootRole || user?.role || 'analyst').toLowerCase();
  ...
  const handleInvite = async (e) => {
    ...
    try {
        const response = await apiClient.post('/auth/invite', invitePayload);
        const roleLabel = selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1);
        if (!response.data?.activation_url) {
            throw new Error("Invitation link was not returned by the server.");
        }
        setInviteResult({
        ...
        if (response.data.email_queued) {
            toast.success(`${roleLabel} invitation created. Secure setup email queued.`);
        } else {
            toast.info(`${roleLabel} invitation created. Share the one-time setup link securely.`);
        }
        fetchTeam(); // 🚀 DYNAMIC: Refresh the team list after invite
    } catch (error) {
        toast.error(formatApiError(error, "Invitation could not be created. Please try again."));
  ...
  const resetForm = () => {
      setEmail(''); setSelectedRole('analyst'); setAllowedPacks({fbr: false, peca: false});
  ...
  const copyActivationLink = async () => {
      try {
          await navigator.clipboard.writeText(inviteResult.activationUrl);
          toast.success("One-time setup link copied.");
      } catch {
          toast.error("Could not copy the link. Select it manually.");
      }
  };
    ...
    <div className="team-container" style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      <div className="team-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        ...
        {currentUserRole === 'admin' && (
            <button className="btn-primary" onClick={() => { resetForm(); setIsModalOpen(true); }} style={{ display: 'flex', gap: '8px', alignItems: 'center', height: '40px', padding: '0 20px', background: 'var(--success)', color: '#000', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
                          ...
                          <tr key={member._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: '0.2s' }}>
                              ...
                              <td style={{ padding: '16px 20px' }}>
                                  <div style={{ display: 'flex', gap: '8px' }}>
                                      {['admin', 'manager', 'analyst'].includes(member.role) ? (
              ...
              <div className="invite-modal" style={{ background: 'var(--bg-card)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', width: '100%', maxWidth: '450px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
                  ...
                  {inviteResult ? (
                      <div style={{ padding: '24px' }}>
                          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', color: 'var(--success)', marginBottom: '14px' }}>
                              <CheckCircle2 size={20} />
                              <strong>{inviteResult.role} access is pending</strong>
                          </div>
                          <p style={{ color: '#a6b8c8', fontSize: '13px', lineHeight: 1.5, margin: '0 0 14px' }}>
                              Share this one-time setup link with {inviteResult.email} through a trusted channel. It expires after 24 hours and can be used once.
                          </p>
                          <input readOnly value={inviteResult.activationUrl} aria-label="One-time setup link" onFocus={(event) => event.target.select()} style={{ width: '100%', padding: '12px', background: 'var(--bg-dark)', border: '1px solid var(--border)', borderRadius: '8px', color: 'white', fontSize: '12px', marginBottom: '14px' }} />
                          <div style={{ display: 'flex', gap: '12px' }}>
                              <button type="button" onClick={copyActivationLink} style={{ flex: 1, padding: '12px', background: 'var(--primary)', border: 'none', color: 'white', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}><Copy size={16} /> Copy Link</button>
                              <button type="button" onClick={closeInviteModal} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid var(--border)', color: '#f8fafc', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>Done</button>
                          </div>
                      </div>
                  ) : (
                  <form onSubmit={handleInvite} style={{ padding: '24px' }}>
                      ...
                      <div style={{ marginBottom: '20px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '10px' }}>
                              {['analyst', 'manager', 'auditor', 'admin'].map(role => (
                      ...
                      <p style={{ margin: '0 0 24px', color: '#7f97ae', fontSize: '12px', lineHeight: 1.5 }}>
                          WarSOC creates a one-time setup link that expires after 24 hours. Email delivery is attempted when available, and the link is also shown once to the administrator. Passwords are never created or shared by an administrator.
                      </p>
                      ...
                      <div style={{ display: 'flex', gap: '12px' }}>
                          <button type="button" onClick={closeInviteModal} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid var(--border)', color: '#f8fafc', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', fontSize: '14px' }}>Cancel</button>
