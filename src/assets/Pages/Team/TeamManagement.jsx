...
export default function TeamManagement() {
  ...
  const fetchTeam = async () => {
    ...
    try {
        ...
        setTeam([]);
        const errMsg = err.response?.data?.detail || "Failed to load team list.";
        toast.error(errMsg);
  ...
  const handleInvite = async (e) => {
    ...
    try {
        ...
        const errMsg = error.response?.data?.detail || error.response?.data?.error || "Network Error. Check console.";
        toast.error(errMsg);
  ...
  const handleRevoke = async (userId, userRole) => {
      ...
      try {
        ...
        fetchTeam(); // 🚀 DYNAMIC: Refresh list
          ...
          const errMsg = error.response?.data?.detail || "Failed to revoke access.";
          toast.error(errMsg);
      }
  };
      ...
      <div className="team-table-card" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.5)' }}>
          <table className="team-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '700px' }}>
              <thead style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border)' }}>
                  <tr>
                      <th style={{ padding: '16px 20px', color: '#94a3b8', fontSize: '13px' }}>User Entity</th>
                      <th style={{ padding: '16px 20px', color: '#94a3b8', fontSize: '13px' }}>Security Role</th>
                      <th style={{ padding: '16px 20px', color: '#94a3b8', fontSize: '13px' }}>System Scope</th>
                      <th style={{ padding: '16px 20px', color: '#94a3b8', fontSize: '13px', textAlign: 'right' }}>Actions</th>
                  </tr>
              </thead>
              <tbody>
                  {loadingList ? (
                      <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Loading team entity list...</td></tr>
                  ) : team.length === 0 ? (
                      <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>No team members found for this organization.</td></tr>
                  ) : (
                      team.map(member => (
                          <tr key={member._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: '0.2s' }}>
                              <td style={{ padding: '16px 20px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                      <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(71, 165, 239, 0.1)', color: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '15px' }}>
                              ...
                              <td style={{ padding: '16px 20px' }}>
                                  <span style={{ 
                                      ...
                                      background: member.role === 'admin' ? 'rgba(239, 68, 68, 0.1)' : member.role === 'auditor' ? 'rgba(245, 158, 11, 0.1)' : 'rgba(71, 165, 239, 0.1)',
                                      color: member.role === 'admin' ? '#ef4444' : member.role === 'auditor' ? '#f59e0b' : 'var(--primary)',
                                      border: `1px solid ${member.role === 'admin' ? 'rgba(239, 68, 68, 0.2)' : member.role === 'auditor' ? 'rgba(245, 158, 11, 0.2)' : 'rgba(71, 165, 239, 0.2)'}`
                                   }}>
                                      {member.role || 'Member'}
                                  </span>
                              </td>
                              <td style={{ padding: '16px 20px' }}>
                                  <div style={{ display: 'flex', gap: '8px' }}>
                                      {member.role === 'admin' || member.role === 'analyst' ? (
                                          <span style={{ color: '#94a3b8', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '6px' }}><Activity size={14}/> Full Internal Network</span>
                                      ) : member.compliance_packs && member.compliance_packs.length > 0 ? (
                                          <>
                                              {member.compliance_packs.includes("fbr_pos_shield") && <span style={{ fontSize: '12px', background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', padding: '2px 8px', borderRadius: '4px', border: '1px solid rgba(16, 185, 129, 0.2)' }}>FBR Logs</span>}
                                              {member.compliance_packs.includes("peca_vault") && <span style={{ fontSize: '12px', background: 'rgba(139, 92, 246, 0.1)', color: '#c4b5fd', padding: '2px 8px', borderRadius: '4px', border: '1px solid rgba(139, 92, 246, 0.2)' }}>PECA Vault</span>}
                                          </>
                                      ) : <span style={{color:'#64748b', fontSize: '13px'}}>No specific tags</span>}
                                  </div>
                              </td>
                              <td style={{ padding: '16px 20px', textAlign: 'right' }}>
                                  {currentUserRole === 'admin' && member.role !== 'admin' && (
                                      <button onClick={() => handleRevoke(member._id, member.role)} style={{ background: 'transparent', border: 'none', color: '#64748b', cursor: 'pointer', padding: '6px', transition: '0.2s', borderRadius: '6px' }} onMouseOver={e => {e.currentTarget.style.color = '#ef4444'; e.currentTarget.style.background = 'rgba(239, 68, 68, 0.1)'}} onMouseOut={e => {e.currentTarget.style.color = '#64748b'; e.currentTarget.style.background = 'transparent'}}>
                                          <Trash2 size={18} />
                                      </button>
                                  )}
                              </td>
          ...
          <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div className="invite-modal" style={{ background: 'var(--bg-card)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', width: '100%', maxWidth: '450px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
                  <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.01)' }}>
                      ...
                      <button type="button" onClick={() => setIsModalOpen(false)} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer' }}><X size={20} /></button>
                  </div>
                  ...
                  <form onSubmit={handleInvite} style={{ padding: '24px' }}>
                      ...
                      <div style={{ marginBottom: '20px' }}>
                          <label style={{ display: 'block', color: '#94a3b8', fontSize: '13px', marginBottom: '8px', fontWeight: '600' }}>Assign Security Role Claims</label>
                          <div style={{ display: 'flex', gap: '10px' }}>
                              {['analyst', 'auditor', 'admin'].map(role => (
                                  <div key={role} onClick={() => setSelectedRole(role)} style={{ 
                                      ...
                                      background: selectedRole === role ? 'rgba(71, 165, 239, 0.1)' : 'transparent',
                                      borderColor: selectedRole === role ? 'var(--primary)' : 'var(--border)',
                                      color: selectedRole === role ? 'var(--primary)' : '#94a3b8'
                                  }}>
                                      {role}
                                  </div>
                              ))}
                      ...
                      <div style={{ marginBottom: '20px' }}>
                          <label style={{ display: 'block', color: '#94a3b8', fontSize: '13px', marginBottom: '8px', fontWeight: '600' }}><Mail size={14} style={{verticalAlign:'middle', marginRight:'4px'}}/> User Entity Email</label>
                      ...
                      <div style={{ marginBottom: '24px' }}>
                          <label style={{ display: 'block', color: '#94a3b8', fontSize: '13px', marginBottom: '8px', fontWeight: '600' }}><Lock size={14} style={{verticalAlign:'middle', marginRight:'4px'}}/> Temporary System Password</label>
                          <input type="text" required placeholder="Provide a strong temporary secret" value={password} onChange={e => setPassword(e.target.value)} 
                              style={{ width: '100%', padding: '12px', background: 'var(--bg-dark)', border: '1px solid var(--border)', borderRadius: '8px', color: 'white', outline: 'none', fontSize: '14px' }} />
                          <small style={{display:'block', marginTop:'6px', color:'#64748b', fontSize:'11px'}}>Share this password securely. User will be forced to change on first login.</small>
                      </div>
                      
                      {/* SCOPE SELECTION (ONLY FOR AUDITORS) */}
                      {selectedRole === 'auditor' && (
                          <div style={{ marginBottom: '24px', padding: '16px', background: 'rgba(245, 158, 11, 0.03)', borderRadius: '8px', border: '1px solid rgba(245, 158, 11, 0.15)' }}>
                              <label style={{ display: 'block', color: '#f59e0b', fontSize: '13px', marginBottom: '12px', fontWeight: '700' }}><Briefcase size={14} style={{verticalAlign:'middle', marginRight:'4px'}}/> Strict Data Scope (Auditor Claims)</label>
