...
export default function AuditorDashboard() {
  ...
  useEffect(() => {
    const fetchRealLogs = async () => {
      ...
      // Dynamic Client Info Set
      setTenantInfo({
          name: user.tenant_id || "Target Organization",
          packs: (user.compliance_packs || []).map(p => p.replace(/_/g, " ").toUpperCase()).join(" + ") || "No Active Packs"
      });

      try {
        // Hit backend with cookie-based auth
                        ...
                        <tbody>
                            {logs.map((log, index) => (
                                <tr key={log.id || index}>
                                    ...
                                    <td className="hash" title={log.hash || "Legacy/Unsealed"}>
                                        {log.hash ? log.hash : <span style={{color: '#ff4444', fontStyle: 'italic', fontWeight: 'bold'}}>Legacy/Unsealed</span>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
